...
from database.models import Banner, Cart, Category, Product, User, Preorder
...
############### Работа c предзаказами ###############
async def orm_add_preorder(session: AsyncSession, data: dict):
    obj = Preorder(
        name=data["name"],
...
async def orm_get_preorders(session: AsyncSession):
    query = select(Preorder)
    result = await session.execute(query)
    return result.scalars().all()


async def orm_get_preorder(session: AsyncSession, preorder_id: int):
    query = select(Preorder).where(Preorder.id == preorder_id)
    result = await session.execute(query)
    return result.scalar()


async def orm_delete_preorder(session: AsyncSession, preorder_id: int):
    query = delete(Preorder).where(Preorder.id == preorder_id)
...
async def orm_change_description(session: AsyncSession, name: str, description: str):
...
async def orm_get_product_name(session: AsyncSession, product_name: str):
    query = select(Product).where(Product.name.like("%" + product_name + "%"))
    result = await session.execute(query)
    return result.scalars().all()
